...
data = model.simulate(paradigm, weights=weights, noise=.5)
test = model.fit_weights(paradigm, data, l2_cost=0.0)
data2 = model.simulate(paradigm, weights=weights, noise=.5)
...
model.fit_residuals(paradigm, data)
...
p, map_ = model.get_stimulus_posterior(data, stimulus_range)
r_decode = get_r(map_, paradigm)

p, map_ = model.get_stimulus_posterior(data2, stimulus_range)
