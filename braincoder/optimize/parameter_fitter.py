...
class ParameterFitter(object):
    ...
    def fit(self, max_n_iterations=1000,
        ...
        n_meaningful_ts = ops.sum(ops.cast(meaningful_ts, 'int32'))

        # Voxel-Specific Parameter Updates
        voxel_parameter_update_ix, update_parameter_ix = tf.meshgrid(
            tf.cast(tf.where(meaningful_ts), tf.int32), voxel_specific_parameters_ix)
        voxel_parameter_update_ix = tf.stack((
            tf.reshape(voxel_parameter_update_ix, tf.size(voxel_parameter_update_ix)),
            tf.reshape(update_parameter_ix, tf.size(update_parameter_ix))), 1)

        print(f'Number of problematic voxels (mask): {ops.sum(ops.cast(meaningful_ts == False, "int32"))}')
        print(f'Number of voxels remaining (mask): {ops.sum(ops.cast(meaningful_ts == True, "int32"))}')
        ...
        if shared_pars is not None:
            ...
            if len(shared_parameter_ix.shape) == 0:
                shared_parameter_init_values = shared_parameter_init_values[:, None]
            ...
            voxel_indices = tf.range(n_voxels, dtype=tf.int32)[:, None]

            voxel_indices, shared_parameter_ix = tf.meshgrid(voxel_indices, shared_parameter_ix, indexing="ij")
            shared_parameter_ix_expanded = tf.reshape(shared_parameter_ix, [-1])
            voxel_indices_expanded = tf.reshape(voxel_indices, [-1])

            shared_parameter_update_ix = tf.stack([voxel_indices_expanded, shared_parameter_ix_expanded], axis=-1)
            ...
            with tf.GradientTape() as tape:
                ...
                if shared_pars is not None:
                    tiled_trainable_shared_parameters = tf.reshape(
                        tf.tile(trainable_shared_parameters[None, :], [n_voxels, 1]),
                        [-1]
                    )
                    parameters = tf.tensor_scatter_nd_update(
                        parameters,
    ...
    def fit_grid(self, *args, fixed_pars=None,
        ...
        if use_correlation_cost:
            ...
            @tf.function
            def _cost(par_grid):
                grid_predictions = self.model._predict(paradigm_[None, ...],
                                                       par_grid[None, ...], None)

                grid_predictions_demeaned = grid_predictions[0] - ops.mean(grid_predictions[0], axis=0, keepdims=True)
                ssq_predictions = ops.sum(grid_predictions_demeaned**2, axis=0, keepdims=True)

                r = ops.sum(grid_predictions_demeaned[:, None, :]*data_demeaned[:, :, None], axis=0, keepdims=True) / ops.sqrt(ssq_predictions[:, None,:]*ssq_data[:, :, None])
                r = r[0]
        ...
        for chunk, pg in tqdm(par_grid.groupby('chunk')):
            ...
            gather_ix = tf.stack((vox_ix, tf.cast(best_ix, tf.int64)), 1)
            best_cost[:, chunk] = tf.gather_nd(cost_, gather_ix)
            best_pars[:, chunk] = tf.gather(pg.values, best_ix)

        best_chunks = tf.cast(ops.argmin(best_cost, axis=1), tf.int64)
    ...
    def refine_baseline_and_amplitude(self, parameters, n_iterations=1, positive_amplitude=True, l2_alpha=1e-3):
        ...
        Y_ = ops.sum(beta[:, None, :] * X, axis=2).numpy().T
    ...
    def _partly_fit_grid(self, fixed_pars, n_voxels, chunk_size, **kwargs):
        ...
        best_chunks = ops.argmin(best_ssq, axis=1)
        best_pars = tf.gather_nd(
            best_pars, tf.stack((vox_ix, best_chunks), axis=1))
